(function(window, document, undefined) {
  ...
  (function(WaltUtils) {
    ...
    (function(Math) {
      ...
      function realAnimTime(duration, delay, count) {
        ...
        if (duration.indexOf('ms') > 0 || (duration.indexOf('ms') < 0 && duration.indexOf('s') < 0)) {} else if (duration.indexOf('s') > 0) {
          durationTime *= 1000;
        }
        var delayTime = parseFloat(delay);
        if (delay.indexOf('ms') > 0 || (delay.indexOf('ms') < 0 && delay.indexOf('s') < 0)) {} else if (delay.indexOf('s') > 0) {
          delayTime *= 1000;
        }
    ...
    (function(CSS) {
      ...
      function handleProp(prop, value) {
          ...
          var compiledCSS = {},
            supportedBrowsers = ['moz', 'webkit', 'ms'];
          // no prefix
  ...
  (function(Walt) {
    var Utils = WaltUtils,
      supportsCss = Utils.CSS.supportsAnimations(),
      $body = $(document.body);
    Walt.DEFAULTS = {
    ...
    function animate(options) {
      ...
      if (options.hasOwnProperty('el')) {
        ...
        console && console.warn && console.warn('Walt - no El provided in animate');
        return;
      }
      // No animation provided
      if (!options.hasOwnProperty('animation') || options.animation === '') {
        console && console.warn && console.warn('Walt - no Animation provided in animate');
        return;
      }
      ...
      if (!supportsCss || bodyIsBlurred) {
        ...
        $el.on('animationend webkitAnimationEnd oAnimationEnd MSAnimationEnd', (function(e) {
          return onAnimateEnd($el, options);
        })).css(objectCSS).addClass('walt-animate animated ' + options.animation);
      }
    ...
    function onAnimateEnd($el, options) {
        ...
        $el.removeClass('walt-animate animated').removeClass(options.animation);
